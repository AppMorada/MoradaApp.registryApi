...
describe('Read condominium data E2E', () => {
	...
	it('should be able to read a condominium data - /condominium/my', async () => {
		...
		expect(readMyCondominiumData.body?.condominiums[0]?.id).toEqual(
			condominium?.id,
		);
		...
		expect(readMyCondominiumData.body?.condominiums[0]?.CNPJ).toEqual(
			condominium?.CNPJ,
		);
		expect(
			typeof readMyCondominiumData.body?.condominiums[0]?.seedKey,
		).toEqual('string');
		expect(readMyCondominiumData.body?.condominiums[0]?.createdAt).toEqual(
			condominium?.createdAt,
		);
		expect(readMyCondominiumData.body?.condominiums[0]?.updatedAt).toEqual(
