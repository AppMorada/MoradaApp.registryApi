...
describe('Update enterprise member E2E test', () => {
	...
	it('should be update a condominium member', async () => {
		...
		const res1 = await supertest(sut.getHttpServer())
			.patch(endpoint(res.body?.condominium?.id, member1.id.value))
			.set('content-type', 'application/json')
			.set('authorization', `Bearer ${res.body?.accessToken}`)
			.send({
				name: 'new name',
				CPF: '273.513.465-26',
				phoneNumber: '3165659898',
			});
	...
	it('should be able to throw a 400 error', async () => {
		...
		const res1 = await supertest(sut.getHttpServer())
			.patch(endpoint('wrong-uuid', 'wrong-uuid'))
			.set('content-type', 'application/json')
			.set('authorization', `Bearer ${res.body?.accessToken}`)
			.send();
	...
	it('should be able to throw a 404 error', async () => {
		...
		const res1 = await supertest(sut.getHttpServer())
			.patch(endpoint(UUID.genV4().value, UUID.genV4().value))
			.set('content-type', 'application/json')
			.set('authorization', `Bearer ${res.body?.accessToken}`)
			.send({
				name: 'new name',
