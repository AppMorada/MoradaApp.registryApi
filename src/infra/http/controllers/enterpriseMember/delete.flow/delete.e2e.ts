...
describe('Delete enterprise member E2E test', () => {
	...
	it('should be delete a enterprise user', async () => {
		...
		const res1 = await supertest(sut.getHttpServer())
			.delete(endpoint(res.body?.condominium?.id, member1.id.value))
	...
	it('should be able to throw a 400 error', async () => {
		...
		const res1 = await supertest(sut.getHttpServer())
			.delete(endpoint('wrong-uuid', 'wrong-uuid'))
			.set('authorization', `Bearer ${res.body?.accessToken}`);

		expect(res1.statusCode).toEqual(400);
		expect(res1.body?.statusCode).toEqual(400);
	...
	it('should be able to throw a 401 error', async () => {
		...
		const res1 = await supertest(sut.getHttpServer()).delete(
	...
	it('should be able to throw a 404 error', async () => {
		...
		const res1 = await supertest(sut.getHttpServer())
			.delete(endpoint(UUID.genV4().value, UUID.genV4().value))
			.set('authorization', `Bearer ${res.body?.accessToken}`);
