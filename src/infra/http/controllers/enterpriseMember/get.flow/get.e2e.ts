...
import { EnterpriseMemberModule } from '../index.module';
...
describe('Get all enterprise member E2E test', () => {
	let sut: INestApplication;
	let memberRepo: EnterpriseMemberRepo;
	const endpoint = (condominiumId: string, userId: string) =>
		`/condominium/${condominiumId}/enterprise-user/${userId}`;
	...
	beforeAll(async () => {
		...
		memberRepo = sut.get(EnterpriseMemberRepo);
		await sut.init();
	});
	...
	it('should get a condominium member list', async () => {
		...
		await memberRepo.create({ user: user1, member });

		const res1 = await supertest(sut.getHttpServer())
			.get(endpoint(res.body?.condominium?.id, member.id.value))
			.set('authorization', `Bearer ${res.body?.accessToken}`);

		expect(res1.statusCode).toEqual(200);
		expect(typeof res1.body?.content?.id).toEqual('string');
		expect(typeof res1.body?.content?.userId).toEqual('string');
		expect(typeof res1.body?.content?.condominiumId).toEqual('string');
		expect(typeof res1.body?.content?.hierarchy).toEqual('number');
		expect(typeof res1.body?.content?.createdAt).toEqual('string');
		expect(typeof res1.body?.content?.updatedAt).toEqual('string');

		expect(typeof res1.body?.content?.userData?.name).toEqual('string');
		expect(typeof res1.body?.content?.userData?.email).toEqual('string');
		expect(typeof res1.body?.content?.userData?.phoneNumber).toEqual(
			'string',
		);
		expect(typeof res1.body?.content?.userData?.CPF).toEqual('string');
		expect(typeof res1.body?.content?.userData?.createdAt).toEqual(
			'string',
		);
		expect(typeof res1.body?.content?.userData?.updatedAt).toEqual(
			'string',
		);
	}, 8000);
	...
	it('should be able to throw a 400 error', async () => {
		...
		const member1 = enterpriseMemberFactory({
			userId: user1.id.value,
			condominiumId: res1.body?.condominium?.id,
		});
		await memberRepo.create({ user: user1, member: member1 });

		const res2 = await supertest(sut.getHttpServer())
			.get(endpoint('wrong-uuid', 'wrong-uuid'))
	...
	it('should be able to throw a 401 error', async () => {
		...
		const res1 = await supertest(sut.getHttpServer()).get(
			endpoint(res.body?.condominium?.id, member1.id.value),
		);
	...
	it('should be able to throw a 404 error', async () => {
		...
		const res2 = await supertest(sut.getHttpServer())
			.get(endpoint(UUID.genV4().value, UUID.genV4().value))
			.set('authorization', `Bearer ${res1.body?.accessToken}`);
