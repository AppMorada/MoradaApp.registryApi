...
describe('Upload a collection of condominium members E2E test', () => {
	let sut: INestApplication;
	const endpoint = (id: string) => `/condominium/${id}/invite/batch`;
	...
	it('should be upload a collection of condominium members', async () => {
		...
		const user1 = userFactory({ CPF: '423.912.075-65' });
		const member1 = condominiumMemberFactory({ userId: user1.id.value });
		const res1 = await supertest(sut.getHttpServer())
			.post(endpoint(res.body?.condominium?.id))
			.set('content-type', 'application/json')
			.set('authorization', `Bearer ${res.body?.accessToken}`)
			.send({
				members: [
					{
						c_email: user1.email.value,
						CPF: user1.CPF.value,
						apartmentNumber: member1.apartmentNumber!.value,
						block: member1.block!.value,
					},
				],
			});
	...
	it('should be able to throw a 404 error', async () => {
		...
		const member1 = condominiumMemberFactory({ userId: user1.id.value });
		const res1 = await supertest(sut.getHttpServer())
			.post(endpoint(UUID.genV4().value))
			.set('authorization', `Bearer ${res.body?.accessToken}`)
			.set('content-type', 'application/json')
			.send({
				members: [
