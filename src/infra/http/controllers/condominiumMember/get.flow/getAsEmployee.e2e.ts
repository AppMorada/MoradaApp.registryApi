...
describe('Get as employee condominium member E2E test', () => {
	let sut: INestApplication;
	let memberRepo: CondominiumMemberRepo;
	const endpoint = (condominiumId: string, memberId: string) =>
		`/condominium/${condominiumId}/as-employee/condominium-member/${memberId}`;
	...
	it('should get a condominium member', async () => {
		...
		const res1 = await supertest(sut.getHttpServer())
			.get(endpoint(res.body?.condominium?.id, member1.id.value))
			.set('authorization', `Bearer ${res.body?.accessToken}`);

		expect(res1.statusCode).toEqual(200);
		expect(typeof res1.body?.content?.id).toEqual('string');
		expect(res1.body?.content?.userId).toBeNull();
		expect(typeof res1.body?.content?.condominiumId).toEqual('string');
		expect(typeof res1.body?.content?.autoEdit).toEqual('boolean');
		expect(typeof res1.body?.content?.hierarchy).toEqual('number');
		expect(typeof res1.body?.content?.c_email).toEqual('string');
		expect(typeof res1.body?.content?.block).toEqual('string');
		expect(typeof res1.body?.content?.apartmentNumber).toEqual('number');
		expect(typeof res1.body?.content?.createdAt).toEqual('string');
		expect(typeof res1.body?.content?.updatedAt).toEqual('string');

		expect(res1.body?.content?.userData).toBeNull();
	});

	it('should return a null value on content field', async () => {
		const user = userFactory();
		const condominium = condominiumFactory();
		...
		const res1 = await supertest(sut.getHttpServer())
			.get(endpoint(res.body?.condominium?.id, UUID.genV4().value))
			.set('authorization', `Bearer ${res.body?.accessToken}`);

		expect(res1.statusCode).toEqual(200);
		expect(res1.body?.content).toBeNull();
	});
	...
	it('should be able to throw a 400 error', async () => {
		...
		const res2 = await supertest(sut.getHttpServer())
			.get(endpoint(res1.body?.condominium?.id, 'wrong-uuid'))
