...
describe('Update condominium E2E test', () => {
	...
	it('should update a condominium', async () => {
		...
		const res2 = await supertest(sut.getHttpServer())
			.patch(endpoint(res1.body?.condominium?.id))
			.set('content-type', 'application/json')
			.set('authorization', `Bearer ${res1.body?.accessToken}`)
			.send({
				name: 'Meu novo condomínio',
				CEP: '76962-053',
				num: 1450,
			});
		...
		const searchedCondominium = await condominiumRepo.find({
			key: new UUID(res1.body?.condominium?.id),
		});
		expect(searchedCondominium.name.value).toEqual('Meu novo condomínio');
		expect(searchedCondominium.CEP.value).toEqual('76962053');
		expect(searchedCondominium.num.value).toEqual(1450);
	});
	...
	it('should be able to throw a 404 error', async () => {
		...
		const res2 = await supertest(sut.getHttpServer())
			.patch(endpoint(UUID.genV4().value))
			.set('content-type', 'application/json')
			.set('authorization', `Bearer ${res1.body?.accessToken}`)
			.send({
				name: 'Meu novo condomínio',
