...
describe('Create condominium E2E test', () => {
	...
	it('should be create a condominium', async () => {
		...
		expect(typeof res1.body?.accessToken).toEqual('string');
		expect(
			typeof res1.headers['set-cookie'][0]?.split('refresh-token=')[1],
		).toEqual('string');

		const res2 = await supertest(sut.getHttpServer())
			.delete(endpoint(res1.body?.condominium?.id))
	...
	it('should be able to throw a 401 error', async () => {
		...
		const res2 = await supertest(sut.getHttpServer()).delete(
			endpoint(res1.body?.condominium?.id),
		);
